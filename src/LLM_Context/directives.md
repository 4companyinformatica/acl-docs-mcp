...
* O ACL não suporta que comandos ou funções sejam separados em múltiplas linhas (`ACCESSDATA64` e `ACCESSDATA32` é a única exceção), ou seja, cada comando ou função deve ser escrito em uma única linha.
...
* Os comentários em ACL, podem ser feitos para uma linha ou para um bloco (os métodos não se misturam), da seguinte forma:
